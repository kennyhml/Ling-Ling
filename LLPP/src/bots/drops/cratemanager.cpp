...
namespace llpp::bots::drops
{
    CrateManager::CrateManager(std::string t_prefix,
                               ...
                               t_grouped_crates, const std::chrono::minutes t_interval,
                               suicide::SuicideStation* t_suicide) :
        prefix(std::move(t_prefix)), align_bed(prefix + "::ALIGN"),
    ...
    void CrateManager::CrateGroupStatistics::add_looted()
    {
        ...
        std::chrono::seconds elapsedTime = (last_looted != UNDEFINED_TIME)
                                               ? util::get_elapsed<std::chrono::seconds>(
                                                   last_looted)
                                               : std::chrono::minutes(30);

        avg_respawn = ((avg_respawn * times_looted) + elapsedTime) / (times_looted + 1);
    ...
    bool CrateManager::run()
    {
        ...
        teleport_to_dropoff();
        if (wasAnyLooted) { dropoff_items(last_known_vault_fill_level); }

        access_bed_above();
        if (suicide) { suicide->complete(); }

        send_summary_embed(prefix, util::get_elapsed<std::chrono::seconds>(start),
                           stats_per_group, last_known_vault_fill_level * 100,
                           std::chrono::system_clock::now() + crates[0][0].
    ...
    bool CrateManager::is_ready_to_run() const { return crates[0][0].is_ready(); }
    ...
    void CrateManager::dropoff_items(float& filledLevelOut)
    {
        ...
        filledLevelOut = dropoff_vault.get_slot_count() / dropoff_vault.get_max_slots();
        ...
        std::chrono::minutes interval)
    {
        ...
        for (size_t i = 0; i < groups.size(); i++) {
            for (QualityFlags qualities : groups[i]) {
                std::string name = util::add_num_to_prefix(prefix + "::DROP", ++n);
