...
namespace llpp::bots::drops
{
		...
		std::map<std::string, bool> cherry_picked)
	{
		...
		message.add_file("image.png", fdata, "image/png ").add_embed(embed);
		...
		const std::chrono::system_clock::time_point next_completion)
	{
		...
		const auto fmt_stamp = std::format(
			"<t:{}:R>", std::chrono::system_clock::to_time_t(next_completion));
		...
		auto embed = dpp::embed()
						 ...
						 .add_field("Time taken:", fmt_taken, true)
						 .add_field("Next completion:", fmt_stamp, true);
		...
		for (auto [vault_name, level] : vault_fill_levels) {
			vault_data += std::format(
				"{}: {}%\n", vault_name, static_cast<int>(level * 100));
		...
		for (int i = 0; i < static_cast<int>(stats.size()); i++) {
			...
			embed.add_field("",
				std::format(">>> **__Group {}:__** \nRespawn: {} "
							"\nTimes looted: {}",
					i + 1, respawn, stats[i].get_times_looted()),
				true);
		}
