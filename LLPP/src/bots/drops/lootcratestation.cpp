...
namespace llpp::bots::drops
{
		...
		std::vector<Item> get_priority_order()
		{
			static std::vector<Item> priority_order{
				...
				ONLY_JM_MC_BPS("Flak Helmet") ONLY_JM_MC_BPS("Flak Chestpiece")
					ONLY_JM_MC_BPS("Flak Leggings") ONLY_JM_MC_BPS(
						"Flak Gauntlets") ONLY_JM_MC_BPS("Flak Boots")
						ONLY_JM_MC_BPS("Fabricated Sniper Rifle")
							ONLY_JM_MC_BPS("Compound Bow")

				// saddles
				MC_ASC_CRAFTED_JM_MC_BP(
					"Giganotosaurus Saddle") MC_ASC_CRAFTED_JM_MC_BP("Carcharo "
																	 "Saddle")
					MC_ASC_CRAFTED_JM_MC_BP("Paracer Platform Saddle")
						MC_ASC_CRAFTED_JM_MC_BP(
							"Tusoteuthis Saddle") MC_ASC_CRAFTED_JM_MC_BP("Basi"
																		  "losa"
																		  "urus"
																		  " Sad"
																		  "dle")
				...
				// grindables
				ANY_CRAFTED("Electric Prod") ANY_CRAFTED("Fabricated Sniper "
														 "Rifle") ANY_CRAFTED(
					"Compound Bow") ANY_CRAFTED("Pump-Action Shotgun")
					ANY_CRAFTED("Assault Rifle") ANY_CRAFTED(
						"Fabricated Pistol") ANY_CRAFTED("Riot Shield")
						ANY_CRAFTED("Riot Helmet") ANY_CRAFTED(
							"Riot Chestpiece") ANY_CRAFTED("Riot Leggings")
							ANY_CRAFTED("Riot Gauntlets") ANY_CRAFTED(
								"Riot Boots") ANY_CRAFTED("Scuba Leggings")
								ANY_CRAFTED("Ghilie Mask") ANY_CRAFTED(
									"Ghilie Chestpiece") ANY_CRAFTED("Ghilie "
																	 "Leggings")
									ANY_CRAFTED("Ghilie Gauntlets") ANY_CRAFTED(
										"Ghilie Boots") ANY_CRAFTED("Harpoon "
																	"Launcher")
										ANY_CRAFTED("Longneck Rifle") ANY_CRAFTED(
											"Simple Pistol") ANY_CRAFTED("Metal"
																		 " Sick"
																		 "le")
											ANY_CRAFTED("Lance") ANY_CRAFTED(
												"Plesiosaur Saddle")
												ANY_CRAFTED(
													"Lance") ANY_CRAFTED("Plesi"
																		 "osaur"
																		 " Plat"
																		 "form "
																		 "Saddl"
																		 "e")
													ANY_CRAFTED("Ankylo Saddle")
														ANY_CRAFTED(
															"Tapejara Saddle")
															ANY_CRAFTED(
																"Kaprosuchus "
	...
	LootCrateStation::LootCrateStation(const std::string& t_name,
		CrateManagerConfig& t_config, asa::structures::CaveLootCrate t_crate)
		: BaseStation(t_name, std::chrono::minutes(t_config.interval)),
		  config_(t_config), crate_(std::move(t_crate)),
		  teleporter_(asa::structures::Teleporter(name_)),
	...
	core::StationResult LootCrateStation::complete()
	{
		...
		if (!should_reroll()) {
			send_success_embed(
				result, loot_screenshot, quality, ++times_looted_);
		}
		else {
			request_reroll(result, loot_screenshot, quality,
				last_discovered_up_ + max_buff_wait_time_, cherry_picked_items);
		}
		return result;
	}

	void LootCrateStation::loot_crate(
		cv::Mat& screenshot_out, std::map<std::string, bool>& cherry_picked_out)
	{
		...
		if (should_reroll()) {
			...
			do {
				crate_.inventory->transfer_all();
			} while (
				!util::await([this]() { return !crate_.inventory->is_open(); },
	...
	{
		std::cout
			<< "[+] Cherry picking high priority items due to reroll mode...\n";
		...
		for (int slot = 0; slot < items.size() && items.size() > 1; slot++) {
			...
			for (const auto& priority : get_priority_order()) {
				if (item_found) {
					break;
				} // restart the priority iteration
				for (size_t i = 0; i < items.size(); i++) {
					if (!items[i] || *items[i] != priority) {
						continue;
					}

					crate_.inventory->take_slot(static_cast<int>(i));
					items_taken[std::format("{}: {}", i, get_repr(items[i]))] =
						true;
