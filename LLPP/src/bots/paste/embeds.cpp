...
namespace llpp::bots::paste
{
	...
	void send_success_embed(const core::StationResult& data)
	{
		...
		embed.set_color(dpp::colors::gray)
			...
			.add_field("Paste obtained:",
				std::to_string(data.obtained_items.at("Achatina Paste")), true)
			.add_field("Next completion:",
				std::format("<t:{}:R>", nextCompletion), true);

		dpp::message msg = dpp::message(core::discord::infoChannelID, embed);
		...
		const std::string& stationName, const std::string& achatinaName)
	{
		...
		embed.set_color(0xf6c330)
			.set_title(
				std::format("Problem with Achatina at '{}'!", stationName))
		...
		dpp::message message = dpp::message(
			core::discord::infoChannelID, "<@&1181159721433051136>")
