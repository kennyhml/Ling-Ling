...
namespace llpp::gui
{
	namespace
	{
		...
		{
			return { accent_color[0] / 255.f, accent_color[1] / 255.f,
				accent_color[2] / 255.f, a };
		}
	}
		...
		const float rounding, const ImDrawFlags flags)
	{
		...
		window->DrawList->AddRectFilled(
			rect.Min, rect.Max, bg_colour, rounding, flags);

		const ImVec2 icon_start(rect.Min.x + 25 - (icon_size.x / 2),
			rect.GetCenter().y - icon_size.y / 2);
		const auto icon_col = ImGui::GetColorU32(
			ImGuiCol_Text, (anim > 0.3f ? anim : 0.3f));
		window->DrawList->AddText(
			font, font->FontSize - 2, icon_start, icon_col, icon);

		const auto text_start = ImVec2(
			rect.Min.x + 50, rect.GetCenter().y - (label_size.y / 2) + 0.5f);
		const auto text_col = ImGui::GetColorU32(
			ImGuiCol_Text, (anim > 0.3f ? anim : 0.3f));
	...
	bool sub_tab_button(const char* label, const bool selected)
	{
		...
		ImGui::ItemAdd(rect, id);
		ImGui::ItemSize(
			ImVec4(rect.Min.x, rect.Min.y, rect.Max.x + 15, rect.Max.y), 0);

		bool hovered, held;
		ImRect pressable_rect(
			rect.Min.x - 7, rect.Min.y - 7, rect.Max.x + 7, rect.Max.y + 7);
		const bool pressed = ImGui::ButtonBehavior(
			pressable_rect, id, &hovered, &held, NULL);
		...
		window->DrawList->AddText(rect.Min,
			ImGui::GetColorU32(ImGuiCol_Text, max(anim, 0.4f)), label);
		...
		const std::function<void()>& render_content)
	{
		...
		ImGui::BeginChild(
			std::string(name).append(".child").c_str(), ImVec2(width, size.y));

		is_hovered = ImGui::IsMouseHoveringRect(ImGui::GetCurrentWindow()->Pos,
			ImGui::GetCurrentWindow()->Pos + ImVec2(width, size.y));

		ImGui::GetWindowDrawList()->AddRectFilled(
			ImGui::GetCurrentWindow()->Pos,
			ImGui::GetCurrentWindow()->Pos + ImGui::GetCurrentWindow()->Size,
			ImColor(28, 30, 36), ImGui::GetStyle().WindowRounding,
	...
	void draw_general_ark_tabs()
	{
		begin_child("Game Settings", ImVec2(300, ImGui::GetWindowHeight()));
		{
			...
			if (ImGui::InputText(
					"##root_dir", config::general::ark::root_dir.get_ptr())) {
			...
			if (ImGui::Button("Get")) {
				...
				openFolder(selected);
				if (!selected.empty()) {
					config::general::ark::root_dir.set(selected);
				}
			}
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
			...
			path_color = valid ? ImVec4(0.f, 1.f, 0.f, 1.f)
			...
			ImGui::SetNextItemWidth(ImGui::GetWindowWidth() * 0.5f);
			if (ImGui::InputText(
					"##server", config::general::ark::server.get_ptr())) {
	...
	void draw_general_bot_tabs()
	{
		begin_child("Environment", ImVec2(375, ImGui::GetWindowHeight()));
		{
			...
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip(
					"The path to the assets shipped with ling ling.");
			}
			...
			if (ImGui::Button("Get")) {
				...
				openFolder(selected);
				if (!selected.empty()) {
					config::general::bot::assets_dir.set(selected);
				}
			}
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip("Select the directory.");
			}
			bool valid = std::filesystem::exists(
							 config::general::bot::assets_dir.get()) ||
						 exists(std::filesystem::current_path() /
								config::general::bot::assets_dir.get());
			assets_path_col = valid ? ImVec4(0.f, 1.f, 0.f, 1.f)
									: ImVec4(1.f, 0.f, 0.f, 1.f);
			...
			if (ImGui::InputText(
					"##itemdata", config::general::bot::itemdata.get_ptr())) {
				config::general::bot::itemdata.save();
			}
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip(
					"The path to the itemdata shipped with ling ling.");
			}
			valid = std::filesystem::exists(
						config::general::bot::itemdata.get()) ||
					exists(std::filesystem::current_path() /
						   config::general::bot::itemdata.get());
			itemdata_col = valid ? ImVec4(0.f, 1.f, 0.f, 1.f)
								 : ImVec4(1.f, 0.f, 0.f, 1.f);
		}
		end_child();
	...
	void draw_discord_bot_config()
	{
			...
			ImVec2(475 - maintabs_data.width, ImGui::GetWindowHeight()));
		{
			...
			if (ImGui::InputText("##bot_token",
					config::discord::token.get_ptr(),
			...
			if (ImGui::InputText(
					"##guild_id", config::discord::guild.get_ptr())) {
			...
			auto* channel_data =
				config::discord::authorization::channels.get_ptr();
			ImGui::Combo("##command_channels", &selected_channel,
				channel_data->data(), channel_data->size());
			...
			if (new_role_popup) {
				...
				if (ImGui::BeginPopupModal("Add an authorized role",
						&new_role_popup, ImGuiWindowFlags_AlwaysAutoResize)) {
					static char name_buffer[256] = {};
					if (ImGui::IsWindowAppearing()) {
						memset(name_buffer, 0, sizeof(name_buffer));
					}
			...
			if (new_user_popup) {
				ImGui::OpenPopup("Add an authorized role");
				if (ImGui::BeginPopupModal("Add an authorized role",
						&new_user_popup, ImGuiWindowFlags_AlwaysAutoResize)) {
					static char name_buffer[256] = {};
					if (ImGui::IsWindowAppearing()) {
						memset(name_buffer, 0, sizeof(name_buffer));
					}
	...
	void draw_discord_info_tabs()
	{
			...
			ImVec2(475 - maintabs_data.width, ImGui::GetWindowHeight() / 2));
		{
			...
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip("[REQUIRED] - General info will be posted "
	...
	void draw_bots_paste_tabs()
	{
			...
			ImVec2(475 - maintabs_data.width, ImGui::GetWindowHeight()));
		{
			...
			ImGui::SetCursorPos({ 150, 11 });
			if (ImGui::InputText(
					"##paste_prefix", config::bots::paste::prefix.get_ptr())) {
			...
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip(
					...
					"and a gateway bed out of snail render named "
					"[PREFIX]::GATEWAY.");
			}
			...
			*interval = std::clamp(*interval, 5, 150);
			...
			ImGui::SetNextItemWidth(ImGui::GetWindowWidth() * 0.5f);
			ImGui::SetCursorPos({ 150, 135 });
			if (ImGui::InputInt("##paste_render",
					config::bots::paste::load_for.get_ptr(), 1, 5)) {
		...
		begin_child("Advanced", ImVec2(280, ImGui::GetWindowHeight()));
		{
			...
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip(
					...
					"remains inactive.\n"
					"You can toggle its state at runtime or through the "
					"discord bot.");
			}
			...
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip(
					...
					"station 3, execute Task A/B, and then resume C at station "
					"4.\n\n"
	...
	void draw_bots_drops_tab()
	{
			...
			ImVec2(425 - maintabs_data.width, ImGui::GetWindowHeight() * 0.33));
		{
			...
			std::vector<const char*>* data =
				config::bots::drops::managers.get_ptr();
			...
			if (new_name_popup) {
				...
				if (ImGui::BeginPopupModal("Name your new crate manager",
						&new_name_popup, ImGuiWindowFlags_AlwaysAutoResize)) {
					static char name_buffer[256] = {};
					if (ImGui::IsWindowAppearing()) {
						memset(name_buffer, 0, sizeof(name_buffer));
					}
					...
					ImGui::InputText(
						"##InputText", name_buffer, IM_ARRAYSIZE(name_buffer));
					if (ImGui::Button("OK")) {
		...
		if (!config::bots::drops::managers.get_ptr()->empty()) {
			const std::string selected =
				(*config::bots::drops::managers.get_ptr())[selected_manager];
		...
		begin_child("Advanced", ImVec2(280, ImGui::GetWindowHeight()));
		{
			...
			if (active) {
				...
				if (ImGui::Checkbox("Disable completion##crate_manager",
						&active->get_ptr()->disabled)) {
					active->save();
				}
				if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
					ImGui::SetTooltip("Completely disable this crate manager's "
									  "completion.\n\n"
				...
				ImGui::SetNextItemWidth(ImGui::GetWindowWidth() * 0.45f);
				ImGui::SetCursorPos({ 150, 135 });
				if (ImGui::InputInt("##crate_render",
						&active->get_ptr()->render_for, 1, 5)) {
					active->save();
				}
				if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
					ImGui::SetTooltip(
			...
			ImVec2(425 - maintabs_data.width, ImGui::GetWindowHeight() * 0.66));
		{
			if (active) {
				...
				ImGui::SetCursorPos({ 150, 11 });
				if (ImGui::InputText(
						"##crate_prefix", &active->get_ptr()->prefix)) {
					active->save();
				}
				if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
					ImGui::SetTooltip(
				...
				ImGui::SetNextItemWidth(ImGui::GetWindowWidth() * 0.5f);
				ImGui::SetCursorPos({ 150, 73 });
				if (ImGui::InputInt("##crate_interval",
						&active->get_ptr()->interval, 1, 5)) {
					active->save();
				}
				if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
					ImGui::SetTooltip("The interval to complete the stations "
									  "at (in minutes).");
				}
			...
			ImVec2(475 - maintabs_data.width, ImGui::GetWindowHeight()));
		{
			...
			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
				ImGui::SetTooltip(
					...
					"150 crops take 8 hours to grow on 300/% greenhouse "
					"effect.");
			}
		}
		end_child();
	...
	void draw_bots_sap_tabs()
	{
			...
			ImVec2(475 - maintabs_data.width, ImGui::GetWindowHeight()));
		{
			...
			ImGui::SetCursorPos({ 150, 11 });
			if (ImGui::InputText(
					"##sap_prefix", config::bots::sap::prefix.get_ptr())) {
			...
			ImGui::SetNextItemWidth(ImGui::GetWindowWidth() * 0.5f);
			ImGui::SetCursorPos({ 150, 42 });
			if (ImGui::InputInt("##sap_count",
					config::bots::sap::num_stations.get_ptr(), 1, 5)) {
				config::bots::sap::num_stations.save();
			}
			...
			*num_stations = std::clamp(*num_stations, 1, 100);
			...
			ImGui::SetNextItemWidth(ImGui::GetWindowWidth() * 0.5f);
			ImGui::SetCursorPos({ 150, 73 });
			if (ImGui::InputInt("##sap_interval",
					config::bots::sap::interval.get_ptr(), 1, 5)) {
				config::bots::sap::interval.save();
			}
	...
	void begin_child(const char* name, ImVec2 size)
	{
		...
		ImGui::BeginChild(std::string(name).append(".main").c_str(), size,
			false, ImGuiWindowFlags_NoScrollbar);

		// draw the name of this tab
		ImGui::GetWindowDrawList()->AddRectFilled(pos, pos + size,
			ImColor(28 / 255.f, 30 / 255.f, 36 / 255.f, animation), 4);
