...
namespace llpp::core::discord
{
    ...
    void register_slash_command(
        const dpp::slashcommand& cmd, const event_callback_t& callback)
    {
    ...
    void inform_started()
    {
        ...
        bot->message_create(
            dpp::message(dpp::snowflake(info_channel_id), started_embed));
    }
        ...
        const std::exception& error, const std::string& task)
    {
        ...
        dpp::message message = dpp::message(
                dpp::snowflake(info_channel_id), "<@&1181159721433051136>")
