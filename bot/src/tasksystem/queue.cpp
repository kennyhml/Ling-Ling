...
namespace lingling
{
    ...
    void enqueue_task(const std::shared_ptr<task>& new_task)
    {
        ...
        for (auto it = task_queue.rbegin(); it != task_queue.rend(); ++it) {
            if ((*it)->get_priority() < new_task->get_priority()) { continue; }

            // We hit a task with the same priority or higher, put our task right after.
            if (it == task_queue.rbegin()) {
                task_queue.push_back(new_task);
            } else {
                task_queue.insert(it.base() + 1, new_task);
            }
            new_task->set_state(task_state::STATE_ENQUEUED);
            break;
        }

        if (new_task->get_state() != task_state::STATE_ENQUEUED) {
            // Didnt insert the task then the queue is either empty or all tasks in the queue
            // had a lower priority, in either case it is fine to add it to the front.
            task_queue.emplace_front(new_task);
            new_task->set_state(task_state::STATE_ENQUEUED);
        }
    }
